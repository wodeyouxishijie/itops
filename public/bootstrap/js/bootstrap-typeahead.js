...
function ($) {
    ...
    var Typeahead = function (element, options) {
        ...
        this.display_cols = this.options.display.split(',');
    ...
    Typeahead.prototype = {
        ...
        lookup: function (event) {
            ...
            if (that.ajax) {
                ...
                if (typeof that.options.val_replace == 'function') {
                    that.query = that.options.val_replace(that.query);
                }
                ...
                if (!items || !items.length) {
                    if (typeof that.options.data_none == 'function') {
                        that.options.data_none(that);
                    }
                    return that.shown ? that.hide() : that;
                }
...
//            } 
            ...
            items = $.grep(data, function (item) {
                for (var i = 0; i < that.display_cols.length; i++) {
                    return that.matcher(item, that.display_cols.length[i]);
                }
            });
        ...
        //
        matcher: function (itemdata, display) {
            for (var i = 0; i < this.display_cols.length; i++) {
                 if (~itemdata[this.display_cols[i]].toLowerCase().indexOf(this.query.toLowerCase())) {
                     return true;
                 }
            }
            return false;
        ...
        render: function (items) {
            var that = this;
//            var display_cols = that.display.split(',');
            items = $(items).map(function (i, item) {
                for (var j = 0; j < that.display_cols.length; j++) {
                    i = $(that.options.item).attr('data-value', item[that.options.val]);
                    i.find('a:eq(' + j + ')').html(that.highlighter(item[that.display_cols[j]], item));
                }
        ...
        select: function () {
            ...
            this.options.itemSelected($selectedItem, $selectedItem.attr('data-value'), $selectedItem.text(), this.selectData());
            return this.hide();
        },
        
        selectData: function () {
            var $selectedItem = this.$menu.find('.active');
            var id = $selectedItem.attr('data-value');
            for (var i = 0;i < this.source.length; i++) {
                if (id == this.options.source[i][this.options.val]) {
